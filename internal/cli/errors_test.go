...
func TestRunCommandsReturnConfigErrorWhenConfigMissing(t *testing.T) {
	testCases := []struct {
		...
		{name: "changes", args: []string{"changes", "items", "--since", "1"}},
		{name: "schema types", args: []string{"schema", "types"}},
		{name: "schema fields", args: []string{"schema", "fields"}},
		{name: "schema creator types", args: []string{"schema", "creator-types"}},
		{name: "schema fields for", args: []string{"schema", "fields-for", "book"}},
		{name: "schema creator types for", args: []string{"schema", "creator-types-for", "book"}},
		{name: "schema template", args: []string{"schema", "template", "book"}},
...
func TestRunArgumentValidationReturnsUsageError(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			wantStderr: "unknown command: item-type-fields",
		},
		{
			name:       "item type creator types is now unknown command",
			args:       []string{"item-type-creator-types"},
			wantStderr: "unknown command: item-type-creator-types",
		},
		{
