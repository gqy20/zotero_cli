...
func (c *CLI) runCreateItems(args []string) int {
	if isHelpOnly(args) {
		return c.printCommandUsage(usageCreateItems)
	}
	data, version, jsonOutput, err := parseWriteBatchArgs(args, true)
	if err != nil {
		fmt.Fprintln(c.stderr, "error:", err)
		fmt.Fprintln(c.stderr, usageCreateItems)
		return 2
	}
	...
	if jsonOutput {
		return c.writeJSON(jsonResponse{OK: true, Command: "create-items", Data: result})
	}
	fmt.Fprintf(c.stdout, "created %d items at library version %d\n", len(result.Successful), result.LastModifiedVersion)
	return 0
}

func (c *CLI) runUpdateItems(args []string) int {
	if isHelpOnly(args) {
		return c.printCommandUsage(usageUpdateItems)
	}
	data, version, jsonOutput, err := parseWriteBatchArgs(args, false)
	if err != nil {
		fmt.Fprintln(c.stderr, "error:", err)
		fmt.Fprintln(c.stderr, usageUpdateItems)
		return 2
	}
	...
	if jsonOutput {
		return c.writeJSON(jsonResponse{OK: true, Command: "update-items", Data: result})
	}
	fmt.Fprintf(c.stdout, "updated %d items (%d unchanged) at library version %d\n", len(result.Successful), len(result.Unchanged), result.LastModifiedVersion)
	return 0
}

func (c *CLI) runDeleteItems(args []string) int {
	if isHelpOnly(args) {
		return c.printCommandUsage(usageDeleteItems)
	}
	keys, version, _, jsonOutput, err := parseKeysListArgs(args, false, false)
	if err != nil {
		fmt.Fprintln(c.stderr, "error:", err)
		fmt.Fprintln(c.stderr, usageDeleteItems)
		return 2
	}
	...
	if jsonOutput {
		return c.writeJSON(jsonResponse{OK: true, Command: "delete-items", Data: result})
	}
	fmt.Fprintf(c.stdout, "deleted %d items at library version %d\n", len(result.Successful), result.LastModifiedVersion)
	return 0
}
