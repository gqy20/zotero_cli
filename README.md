...
> **国内用户**：GitHub 下载较慢时可使用 [Gitee 镜像](https://gitee.com/gqy20/zotero_cli/releases) 获取 Release 产物，源码和文档完全同步。
...
1. 检测当前平台（Windows/macOS/Linux），下载最新 zot 二进制：
   - GitHub: https://github.com/gqy20/zotero_cli/releases
   - 国内镜像: https://gitee.com/gqy20/zotero_cli/releases
...
| **Gitee（国内推荐）** | 从 [Releases](https://gitee.com/gqy20/zotero_cli/releases) 下载 `zot.exe` |
...
# Gitee（国内更快）:
curl -fsSL https://gitee.com/gqy20/zotero_cli/releases/latest/download/zot-linux-amd64 -o ~/.local/bin/zot && chmod +x ~/.local/bin/zot
...
# GitHub（默认）
_GITHUB_RAW="https://raw.githubusercontent.com/gqy20/zotero_cli/master"
# Gitee（国内更快）
_GITEE_RAW="https://gitee.com/gqy20/zotero_cli/raw/master"

# 选择一个源，以下以 GitHub 为例：
curl -fsSL ${_GITHUB_RAW}/.claude/skills/zotero-cli/SKILL.md \
  -o ~/.claude/skills/zotero-cli/SKILL.md
curl -fsSL ${_GITHUB_RAW}/.claude/skills/zotero-cli/reference.md \
  -o ~/.claude/skills/zotero-cli/reference.md
curl -fsSL ${_GITHUB_RAW}/.claude/skills/zotero-cli/examples/find-output.md \
  -o ~/.claude/skills/zotero-cli/examples/find-output.md
curl -fsSL ${_GITHUB_RAW}/.claude/skills/zotero-cli/examples/show-output.md \
...
| **Windows** | 手动下载 | 从 [GitHub Releases](https://github.com/gqy20/zotero_cli/releases) 或 [Gitee Releases](https://gitee.com/gqy20/zotero_cli/releases) 下载 `zot.exe` → 放入 PATH 目录 |
| **macOS / Linux** | 手动下载 | 从 [Releases](https://github.com/gqy20/zotero_cli/releases)（或 [Gitee 镜像](https://gitee.com/gqy20/zotero_cli/releases)）下载对应二进制 → `chmod +x zot && mv zot /usr/local/bin/` |
| **任意平台** | 源码构建 | `git clone https://github.com/gqy20/zotero_cli.git`（或 `https://gitee.com/gqy20/zotero_cli.git`）→ `go build -o zot ./cmd/zot` |
