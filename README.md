...
| 标注散落在各处无法汇总 | `zot annotations KEY --json` → 双源统一输出，含时间戳 |
...
# 让 AI 写入标注到 PDF
zot annotate KEY --text "homoploid hybrid speciation" --color red --comment "关键定义"
...
| `zot annotations KEY` | SQLite + PyMuPDF 双源读取 | 同时获取你在 Zotero 阅读器里做的标注 **和** PDF 文件内嵌入的标注 |
| `zot annotate KEY` | PyMuPDF 直接写入 PDF | 标注写入 PDF 文件，下次在 Zotero 阅读器中打开即可看到 |
...
| **PDF** | `annotate` | 写入高亮/下划线/笔记标注 |
| **PDF** | `annotations` | 读取/删除 PDF 标注（双源） |
