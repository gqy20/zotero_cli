...
## 8. 实际实施方案：v5b cluster_drawings（2026-04）

> 本节记录最终落地方案，与第 6 节的原始规划有显著差异。

### 8.1 方案变更原因

原计划采用 **pdfcpu 提取 XObject 原图 + Go 后处理过滤**（方案 A），实际验证后发现：

| 问题 | 详情 |
|------|------|
| XObject 图片 ≠ Figure | pdfcpu 提取的是 PDF 内嵌图片对象，学术 PDF 的 Figure 大量由**矢量绘图**构成（非 XObject），导致漏提取严重 |
| 矢量 Figure 主流 | LaTeX/Word 生成的论文中，图表、流程图、坐标系均为矢量路径，XObject 仅占少数（照片类） |
| 过滤规则脆弱 | 基于尺寸/大小的 logo 过滤在不同期刊模板下阈值不稳定 |

### 8.2 最终方案：PyMuPDF cluster_drawings() + 位图锚点回退

**核心决策**：放弃"提取已有图片对象"思路，改为**识别页面区域中的图形内容**。

```
┌─────────────────────────────────────────────┐
│              zot extract-figures            │
│                                             │
│  Go CLI ──stdin──▶ Python (PyMuPDF)        │
│                      │                      │
│              ┌───────┴───────┐               │
│              ▼               ▼               │
│        Path A: 矢量     Path B: 位图        │
│    cluster_drawings()   图片锚点回退         │
│    聚类矢量图形区域      未覆盖的大图         │
│              │               │               │
│              └───────┬───────┘               │
│                      ▼                       │
│              统一过滤链                     │
│   面积 → 锚点 → 文字密度 → Caption → 去重  │
│                      │                       │
│                      ▼                       │
│              渲染为 PNG + Caption 吸附       │
│                      │                       │
...
```python
DEFAULTS = {
    "render_dpi": 200,          # 渲染 DPI
    "x_tolerance": 5,           # 聚类容差 (pt)
    "y_tolerance": 5,
    "min_cluster_area_pt": 5000, # 最小聚类面积
    "min_output_w_px": 120,     # 最小输出宽度 (px)
    "min_output_h_px": 100,     # 最小输出高度 (px)
    "min_output_kb": 15,        # 最小文件大小 (KB)
    "min_pct_page": 8,          # 占页面最小比例 (%)
    "large_area_pct": 15,       # 大面积阈值 (%) — 需要锚点
    "max_text_ratio": 0.35,     # 最大文字占比 — 超过视为文本区
    "anchor_detect_w": 20,      # 检测锚点最小尺寸 (pt)
    "anchor_fallback_h": 80,   # 回退锚点最小尺寸 (pt)
}
```
...
| 论文 | 类型 | 提取数 | 准确率 |
|------|------|--------|--------|
| MBE2015 | 进化生物学（矢量为主） | 5 figs | ✅ 全部正确 |
| AdvSci2025 | 转座子图谱（复杂矢量） | 7 figs | ✅ 全部正确 |
| MBE2021 | 分子进化（混合） | 2 figs | ✅ 全部正确 |
| Mec2024 | 杂交物种（位图照片为主） | 2 figs | ✅ 全部正确 |

### 8.5 与原计划的差异总结

| 维度 | 原计划（pdfcpu XObject） | 实际方案（cluster_drawings v5b） |
|------|--------------------------|-------------------------------|
| 提取目标 | 内嵌图片 XObject 对象 | 页面矢量聚类区域 + 位图锚点 |
| 引擎 | Go (pdfcpu) | Python (PyMuPDF) 子进程 |
| 矢量 Figure | ❌ 无法提取 | ✅ 核心能力 |
| 位图 Figure | ✅ 原始格式透传 | ✅ 锚点回退 |
| Caption 包含 | ❌ 无 | ✅ 自动吸附 |
| 依赖 | 纯 Go | Python 运行时 |
| 许可证风险 | Apache-2.0（无） | AGPL-3.0（仅调用方，不链接） |
