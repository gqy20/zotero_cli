...
### `zot schema` — Zotero 元数据内省

```bash
zot schema types                          # 所有文献类型
zot schema fields                         # 所有可用字段
zot schema creator-types                  # 所有作者角色
zot schema fields-for journalArticle       # 某类型的有效字段
zot schema creator-types-for artwork      # 某类型的有效作者角色
zot schema template book                  # 新建条目的 JSON 模板
```
